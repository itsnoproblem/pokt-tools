...
export const RewardTransaction = (props: RewardTransactionProps) => {
    ...
    return (
        <React.Fragment key={tx.hash}>
            ...
            {!isMobile && (<GridItem padding={2} backgroundColor={props.color} pr={4} textAlign={"right"}>{tx.num_relays.toLocaleString()} relays</GridItem>)}
            <GridItem padding={2} backgroundColor={props.color} pr={2} pl={2} textAlign={"left"}>({tx.chain.id}) {tx.chain.name}</GridItem>
            {!isMobile && (<GridItem padding={2} backgroundColor={props.color} pr={4} textAlign={"right"}>{tx.pokt_per_relay.toFixed(8)}</GridItem>)}
