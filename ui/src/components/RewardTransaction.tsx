...
export const RewardTransaction = (props: RewardTransactionProps) => {
    const tx = props.tx;
    const amount = (tx.type === 'pocketcore/proof') ? '-' : Number(tx.num_relays * tx.pokt_per_relay * tx.stake_weight).toFixed(4) + " POKT";
    ...
    return (
        <React.Fragment key={tx.hash}>
            ...
            {!isMobile && (<GridItem padding={2} backgroundColor={props.color} pr={4} align={"right"}>{tx.stake_weight.toFixed(15)}</GridItem>)}
